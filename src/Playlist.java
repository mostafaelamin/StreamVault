...
public class Playlist {
	...
	public Playlist(String name) {
		if(name == null || name.isBlank()) { // checks if null or blank
			throw new IllegalArgumentException("Input for name doesnt work!");	
		}
		this.name = name;
	...
	public boolean addToPlaylist(String videoTitle) {
		if(videoTitle == null || videoTitle.isBlank()) { //checks if null or blank
	...
	public boolean removeFromPlaylistAll(String videoTitle) {
		...
		// removes all matching titles and returns true if changed
		return this.videoTitles.removeIf(title -> title.equals(videoTitle)); 
	}
	...
	public void shuffleVideoTitles(Random random) {
		if(random == null) {
			...
			Collections.shuffle(this.videoTitles, random); // otherwise use the specified shuffle randomizer
		}
